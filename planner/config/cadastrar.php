...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_verificar) {
        ...
        if ($resultado_verificar->num_rows > 0) {
            ...
            echo "<script>alert('O email já está cadastrado.'); window.location.href = '../../painel.php';</script>";
        } else {
            ...
            if ($stmt_inserir) {
                ...
                if ($stmt_inserir->execute()) {
                    ...
                    echo "<script>alert('Usuário cadastrado com sucesso!'); window.location.href = '../../painel.php';</script>";
                } else {
                    // Exibe uma mensagem de erro se ocorrer falha na inserção
                    echo "<script>alert('Erro ao cadastrar o usuário.'); window.location.href = '../../painel.php';</script>";
                }

                $stmt_inserir->close();
            } else {
                // Se não conseguir preparar a query de inserção
                echo "<script>alert('Erro na preparação da consulta de inserção.'); window.location.href = '../../painel.php';</script>";
            }
        }
        ...
        echo "<script>alert('Erro na preparação da consulta de verificação.'); window.location.href = '../../painel.php';</script>";
    }
